...
public class UserDAO {
  ...
  public static int save(String nombre, String apellidos,String email,String fecha, int tags){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into User (nombre,apellidos,email,fecha,tags) values(?,?,?,?,?)");
...
public static int update(String nombre, String apellidos,String email,String fecha, int tags){
	...
	try{
		Connection con=getConnection();
		PreparedStatement ps=con.prepareStatement("update User set nombre=?,apellidos=?,fecha=?,tags=? where email=?");
		ps.setString(1,nombre);
		ps.setString(2,apellidos);
...
public static Hashtable<String,String> queryByEmail (String email) {
	...
	try {
	    ...
	    ResultSet rs = stmt.executeQuery("select nombre, apellidos, fecha, tags from User where email = " + email);
	    while (rs.next()) {
